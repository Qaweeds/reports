...
class ReportControllerLongsort extends Controller
{
    private $keys;
    private $shuffleData;
    private $shuffled;
    ...
    public function create_table(Request $r)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            foreach ($data[$i] as $key => $cell) {
                ...
                if ($i == 0) {
                    if ($key == 0) $cell = ($date) ? '<a href="' . route('report') . '?sort=m' . '&D=' . $date . '">M</a>' : '<a href="' . route('report') . '?sort=m' . '">M</a>';
                    ...
                    else if ($key == 3) $cell = ($date) ? '<div class="date-wrap"><a href="' . route('report') . '?sort=q' . '&D=' . $date . '">Скольз.</a></div>' :
...
//                    else if ($key != $last_key) $day = $ruDaysArr[Carbon::parse($cell)->format('w')]; // дни недели. на русском
                ...
                if ($key < 4) {
                    switch ($key) {
                        case '0':
                            $t .= ($i) ? '<td class="main-store">' . mb_substr($cell, 0, 2) . '</td>' : '<td class="store-head notgrey bg-white">' . $cell . '</td>';
                            break;
                        case '1':
                            $t .= ($i) ? '<td class="name">' . $this->trimName($data[$i]) . '</td>' : '<td class="fio-head notgrey bg-white">' . $cell . '</td>';
                            break;
                        case '2':
                            $t .= ($i) ? '<td class="trend">trendrow' . $i . '</td>' : '<td class="date notgrey trend sort">' . $cell . '</td>';
                            break;
                        case '3':
                            $t .= ($i) ? '<td class="slice">' . $this->bigAndSmall($cell) . '</td>' : '<td class="slice date notgrey">' . $cell . '</td>';
                            break;
                    }
            ...
            if ($i == 0) {
                ...
                if (count($trends_arr) != 4 or (($trends_arr[2] + $trends_arr[3]) == 0)) {        //ТРЕНДЫ
                    $trend = '';
                } else {
                ...
                $t .= '<td class="current-month-summ total-data-border">' . $this->bigAndSmall($this->totalByPeriod($data[$i][1], $firstDate->subMonth(), 'month')) . '</td>';
                $t .= '<td class="current-season-summ">' . $this->bigAndSmall($this->totalByPeriod($data[$i][1], $firstDate, 'season')) . '</td>';
                $t .= '<td class="current-year-summ">' . $this->bigAndSmall($this->totalByPeriod($data[$i][1], $firstDate, 'year')) . '</td>';
    ...
    public function totalByPeriod($name, Carbon $date, $period)
    {
        switch ($period) {
            case 'week':
